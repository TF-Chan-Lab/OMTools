...
public class Filter implements Copyable<Filter>, SelectableMode {
	...
	private OptMapResultNode trimToPassFilter(OptMapResultNode result) {
		if (optrefmap == null)
			throw new NullPointerException("A reference is needed for trimming in filter module."); // Should do something on it like throwing an exception
	...
	public static void assignOptions(ExtendOptionParser parser, int level) {
		...
		parser.accepts("filtermode", "Filter Mode. 0: No filter; 1: Filter by all the following options; 2: Filter by minimum score only").withRequiredArg().ofType(Integer.class).defaultsTo(0);
		parser.accepts("minmatch", "Minumum number of matches of a partial alignment").withRequiredArg().ofType(Integer.class).defaultsTo(3);
		parser.accepts("maxfp", "Maximum number of extra signals of a partial alignment").withRequiredArg().ofType(Integer.class).defaultsTo(10000);
		parser.accepts("maxfn", "Maximum number missing signals of a partial alignment").withRequiredArg().ofType(Integer.class).defaultsTo(10000);
		parser.accepts("maxfpr", "Maximum rate of extra signals of a partial alignment").withRequiredArg().ofType(Double.class).defaultsTo(1e-4);
		parser.accepts("maxfnr", "Maximum rate of missing signals of a partial alignment").withRequiredArg().ofType(Double.class).defaultsTo(0.5);
		parser.accepts("minscore", "Minimum score of a partial alignment").withRequiredArg().ofType(Double.class).defaultsTo(0.0);
		parser.accepts("minsubfragratio", "Minimum subfragment ratio of a partial alignment").withRequiredArg().ofType(Double.class).defaultsTo(0.0);
		parser.accepts("minsigratio", "Minimum aligned signal ratio of a partial alignment").withRequiredArg().ofType(Double.class).defaultsTo(0.0);

		parser.accepts("trimmode", "Trim Mode. 0: Trim mode disabled; 1: Trim mode enabled").withRequiredArg().ofType(Integer.class).defaultsTo(0);
		parser.accepts("maxtrim", "Maximum number of trimming steps of a partial alignment").withRequiredArg().ofType(Integer.class).defaultsTo(5);
