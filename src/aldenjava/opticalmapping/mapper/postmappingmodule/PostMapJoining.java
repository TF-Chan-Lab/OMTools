...
public class PostMapJoining implements SelectableMode {
	private int overlapmergemode = 0;
	...
	public void setMode(OptionSet options) {
		setMode((int) options.valueOf("overlapmergemode"));
	}

	@Override
	public void setMode(int mode) {
		this.overlapmergemode = mode;
	}

	@Override
	public int getMode() {
		return this.overlapmergemode;
	}

	public void setParameters(OptionSet options) {
		this.setParameters((int) options.valueOf("match"), (int) options.valueOf("fpp"), (int) options.valueOf("fnp"), (boolean) options.valueOf("local"));
	}

	public void setParameters(int matchscore, int falseppenalty, int falsenpenalty, boolean allowLocalAlignment) {
	...
	public PostMapJoining copy() {
		...
		pmj.setMode(overlapmergemode);
		pmj.setParameters(matchscore, falseppenalty, falsenpenalty, allowLocalAlignment);
		return pmj;
	}

	public static void assignOptions(ExtendOptionParser parser, int level) {
		parser.addHeader("Overlapped Alignment Merging Module Options", level);
		parser.accepts("overlapmergemode", "Mode: 0: Disable merging step; 1: Merge same partial alignments; 2: Merge overlapping partial alignments").withRequiredArg().ofType(Integer.class).defaultsTo(2);
