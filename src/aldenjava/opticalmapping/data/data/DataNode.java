...
public class DataNode implements Identifiable<String> {
	...
	public DataNode(DataNode data) {
		init(data.name, data.size, Arrays.copyOf(data.refp, data.refp.length));
	...
	public DataNode(String name, boolean[] zeroOneSeq) {
		...
		init(name, size, ArrayUtils.toPrimitive(refp.toArray(new Long[refp.size()])));
	}
	...
	public DataNode(String name, long size) {
		init(name, size, new long[0]);
	}

	private void init(String name, long size, long[] refp) throws IllegalArgumentException {
		if (name == null)
			throw new IllegalArgumentException("DataNode cannot be initialized with NULL name.");
		if (size < 0)
			throw new IllegalArgumentException("DataNode is initialized with negative size: " + size);
//		if (refp == null)
//			throw new IllegalArgumentException("DataNode cannot be initialized with NULL refp. Use new long[0] to indicate labels are absent.");
		this.name = name;
		this.size = size;
		this.refp = refp;
	}
	...
	public long[] getRefl() {
		long[] refl = new long[this.getTotalSegment()];
	...
	public double[] getSignalDensityAcrossRegion(long regionSize) {
		...
		while (startSig < refp.length && stopSig < refp.length) {
	...
	public List<ReferenceSignal> getReferenceSignals() {
		List<ReferenceSignal> list = new ArrayList<>();
		for (int i = 0; i < refp.length; i++)
			list.add(new ReferenceSignal(this.name, i));
		return list;
	}
	public List<ReferenceSignal> getReferenceSignals(GenomicPosNode region) {
		int index = findRefpIndex(region.start);
		int index2 = findRefpIndex(region.stop);
		if (!hasSignal(region.stop))
			index2--;
		List<ReferenceSignal> list = new ArrayList<>();
	...
	public DataNode subRefNode(String newname, long start, long stop) {
		if (start > stop)
			throw new IllegalArgumentException("Stop is smaller than start when calling subrefnode");
	...
	public Kmer getKmer(int k, int pos) {
		List<Long> sizeList = new ArrayList<Long>();
		for (int i = pos; i < pos + k; i++)
			sizeList.add(getRefl(i));
		return new Kmer(this.name, pos, sizeList);
	}
...
/*
	...
	public void removeSmallSegments(long minSegSize) {
		List<Long> refpList = new ArrayList<>();
		long shift = 0;
		long prev = 0;
		for (long p : this.refp) {
			if (p - prev - 1 >= minSegSize)
				refpList.add(p - shift);
			else
				shift += p - prev;
			prev = p;
		}
		this.refp = ArrayUtils.toPrimitive(refpList.toArray(new Long[refpList.size()]));
		this.size -= shift;
	}
...
//				index2++;
//		}
		int index = this.findRefpIndex(start);
		long stop = start + length - 1;
		int index2 = this.findRefpIndex(stop);
		if (index2 < refp.length && refp[index2] == stop)
			index2++;
	...
	public static Comparator<DataNode> namecomparator = new Comparator<DataNode>() {
		@Override
		public int compare(DataNode d1, DataNode d2) {
