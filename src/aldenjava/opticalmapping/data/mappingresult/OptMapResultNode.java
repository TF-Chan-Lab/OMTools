...
public class OptMapResultNode {
	...
	public void updateMSP() {
		...
		Collections.sort(mspsQ, MatchingSignalPair.qposComparator);
		List<MatchingSignalPair> mspsR = new ArrayList<MatchingSignalPair>(msps);
		Collections.sort(mspsR, MatchingSignalPair.rposComparator);
	...
	/**
	 * Obtain the estimated mappedRegion including the unmapped part
	 * @return
	 */
	public GenomicPosNode getUnmappedRegion() {
		GenomicPosNode moleculeRegion = this.getMoleMappedRegion();
		long start;
		long stop;
		if (mappedstrand == 1) {
			start = (long) (this.mappedRegion.start - (moleculeRegion.start - 1) * this.getMapScale());
			stop = (long) (this.mappedRegion.stop + (this.parentFrag.size - moleculeRegion.stop) * this.getMapScale());
		}
		else {
			start = (long) (this.mappedRegion.start - (this.parentFrag.size - moleculeRegion.stop) * this.getMapScale());
			stop = (long) (this.mappedRegion.stop + (moleculeRegion.start - 1) * this.getMapScale());
		}
		return new GenomicPosNode(mappedRegion.ref, start, stop);
	}
	@Override
